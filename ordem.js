...
window.carregarOrdensServico = async function() {
    ...
    try {
        ...
        tabela.innerHTML = dadosOrdenados.map(os => {
            ...
            // SINO FLUTUANTE (ABSOLUTE): Flutua à esquerda sem empurrar o texto da placa
            let iconeNotificacao = os.lab_atualizado ? 
                `<button type="button" onclick="window.visualizarNotificacaoLab(event, ${os.id}, '${numeroFormatado}', '${os.placa}', '${os.situacao}')" class="absolute left-6 top-1/2 transform -translate-y-1/2 text-red-500 hover:text-red-700 animate-pulse" title="Laboratório enviou atualizações!">
